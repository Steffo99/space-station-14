## SpaceVillainGame

space-villain-game-player-attack-message = You attack {$enemyName} for {$attackAmount} damage!
space-villain-game-player-heal-message = You use {$magicPointAmount} MP to heal {$healAmount} HP!
space-villain-game-player-recharge-message = You regain {$regainedPoints} MP!
space-villain-game-player-wins-message = You won!
space-villain-game-enemy-dies-message = {$enemyName} dies.
space-villain-game-player-loses-message = You lost!
space-villain-game-enemy-cheers-message = {$enemyName} cheers.
space-villain-game-enemy-dies-with-player-message = {$enemyName} dies, but takes you with them.
space-villain-game-enemy-throws-bomb-message = {$enemyName} throws a bomb, exploding you for {$damageReceived} damage!
space-villain-game-enemy-steals-player-power-message = {$enemyName} steals {$stolenAmount} of your MP!
space-villain-game-enemy-heals-message = {$enemyName} heals for {$healedAmount} HP!
space-villain-game-enemy-attacks-message = {$enemyName} attacks you for {$damageDealt} damage!
